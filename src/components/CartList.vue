  ...
  <div>
    ...
    <div v-if="cart.length > 0" class="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-4">
      <div v-for="item in cart" :key="item.id" class="border flex flex-col gap-4 p-4 rounded-lg">
        ...
        <button @click="removeFromCart(item.id)" class="bg-red-500 text-white rounded-lg p-2">
          Remove from Cart
        </button>
      </div>
    </div>
    ...
    <div v-if="cart.length > 0" class="mt-4">
      <p>Total: $ {{ total }}</p>
      <button @click="confirmPurchase" class="bg-green-500 text-white rounded-lg p-2">
        Confirm Purchase
      </button>
    </div>
  </div>
